...
if (!function_exists('module_collect')) {
    /**
     * 获取模块数据层类 获取需要参数
     * @param $name
     * @param string $appendSuffix
     * @return \App\Collect\BaseCollect
     */
    function module_collect($name, $appendSuffix = 'Collect'){
        $class = '\\App\\Collect\\'.studly_case($name).$appendSuffix;
        return new $class();
    }
}
