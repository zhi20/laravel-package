...
class Worker extends Command
{
     ...
     * @var string
     */
    protected $signature = 'swoole:worker {action} {param=worker-001}
                            {--d : Set daemonize}
                            {--log_level= : Set the log level}
                            {--worker_num= : Set the worker num}
                            {--reactor_num= : Set the reactor num}
                            ';
     ...
     * @var string
     */
    protected $description = 'run swoole for worker';
    ...
    public function start()
    {
        ...
        $worker_config = config('swoole.worker')[$key];
        ...
        $server->worker_num = intval($options['worker_num']);
        $server->reactor_num = intval($options['reactor_num']);
    ...
    public function kill()
    {
        ...
        $string = '$(ps aux|grep ' . $project_name . '_Swoole_Worker' . '|grep -v grep|awk ' . '\'{print $2}\'' . ')';
