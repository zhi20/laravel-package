...
class CreateLogAlipayPayTable extends Migration
{
    /**
     * Run the migrations.
     *
    ...
    public function up()
    {
        \Schema::create('log_alipay_pay', function (Blueprint $table) {
            $table->increments('id');
            $table->integer('user_id')->length(11)->unsigned()->comment("用户id")->default("0");
            $table->tinyInteger('type')->length(1)->unsigned()->comment("支付类型:1,充值余额;2,订单支付;")->default("1");
            $table->string('out_trade_no', 225)->comment("订单号")->default("");
    ...
    {
        //Schema::dropIfExists('log_alipay_pay');
    }
}
