...
class WechatOrigin
{
    ...
    const OAUTH_COMPONENT_TOKEN_URL = '/sns/oauth2/component/access_token?';
    const OAUTH_REFRESH_URL = '/sns/oauth2/refresh_token?';
    const OAUTH_COMPONENT_REFRESH_URL = '/sns/oauth2/component/refresh_token?';
    ...
    //企业号
    const API_QIYE_URL_PREFIX = 'https://qyapi.weixin.qq.com/cgi-bin';      //企业接口
    const AUTH_QIYE_URL = '/gettoken?';
    const GET_QIYE_TICKET_URL = '/get_jsapi_ticket?';
    const QIYE_MESSAGE_SEND_URL = '/message/send?';
    const QIYE_DEPARTMENT_LIST_URL = '/department/list?';
    const QIYE_DEPARTMENT_CREATE_URL = '/department/create?';
    const QIYE_DEPARTMENT_UPDATE_URL = '/department/update?';
    const QIYE_DEPARTMENT_DELETE_URL = '/department/delete?';
    const QIYE_GET_AGENT_URL = '/agent/get?';
    const QIYE_SET_AGENT_URL = '/agent/set?';
    const QIYE_GET_USERINFO_URL = '/user/getuserinfo?';
    const QIYE_GET_USERDETAIL_URL = '/user/getuserdetail?';//使用user_ticket获取成员详情
    ...
    public function __construct($options)
    {
        ...
        $this->agentid = isset($options['agentid']) ? $options['agentid'] : false;
    }
...
//		return '{' . $json . '}'; //Return associative JSON
    ...
    public function saveComponentVerifyTicket()
    {
        ...
        $authname = 'wechat_component_verify_ticket' . $appid;
    ...
    public function checkComponentAuth($appid = '', $appsecret = '', $component_access_token = '')
    {
        ...
        $authname = 'wechat_component_access_token' . $appid;
        if ($rs = $this->getCache($authname)) {
     ...
     * @return bool|string
     */
    public function getComponentOAuthUri($redirect_uri)
    {
        ...
        $url = self::COMPONENT_OAUTH_URL . 'component_appid=' . $this->appid . '&pre_auth_code=' . $code['pre_auth_code'] . '&redirect_uri=' . urlencode($redirect_uri);
        return $url;
    }
     ...
     * @param string $appid 用于多个appid时使用
     */
    public function resetComponentJsTicket($appid)
    {
        $this->jsapi_ticket = '';
        $authname = 'wechat:wechat_jsapi_ticket:' . $appid;
        $this->removeCache($authname);
        return true;
    }
     ...
     * @param string $jsapi_ticket 手动指定jsapi_ticket，非必要情况不建议用
     */
    public function getComponentJsTicket($appid, $jsapi_ticket = '')
    {
        if (!$this->access_token && !$this->checkAuth()) return false;
        if ($jsapi_ticket) { //手动指定token，优先使用
            $this->jsapi_ticket = $jsapi_ticket;
     ...
     * @return array|bool 返回签名字串
     */
    public function getComponentJsSign($url, $timestamp = 0, $noncestr = '', $appid)
    {
        if (!$this->jsapi_ticket && !$this->getComponentJsTicket($appid) || !$url) return false;
        if (!$timestamp)
            $timestamp = time();
        if (!$noncestr)
            $noncestr = $this->generateNonceStr();
     ...
     * @return string
     */
    public function getComponentOauthRedirect($callback, $state = '', $scope = 'snsapi_userinfo', $authorizer_appid)
    {
        $params = array(
        ...
        return self::OAUTH_PREFIX . self::OAUTH_AUTHORIZE_URL . http_build_query($params) . '#wechat_redirect';
    }
     ...
     * @return array {access_token,expires_in,refresh_token,openid,scope}
     */
    public function getComponentOauthAccessToken($authorizer_appid)
    {
        if (!$this->component_access_token && !$this->checkComponentAuth()) return false;
        ...
        $result = $this->http_get(self::API_BASE_URL_PREFIX . self::OAUTH_COMPONENT_TOKEN_URL . http_build_query($params));
        if ($result) {
            $json = json_decode($result, true);
            if (!$json || !empty($json['errcode'])) {
                $this->errCode = $json['errcode'];
     ...
     * @return boolean|mixed
     */
    public function getComponentOauthRefreshToken($refresh_token, $authorizer_appid)
    {
        if (!$this->component_access_token && !$this->checkComponentAuth()) return false;
        ...
        $result = $this->http_get(self::API_BASE_URL_PREFIX . self::OAUTH_COMPONENT_REFRESH_URL . http_build_query($params));
        if ($result) {
            $json = json_decode($result, true);
            if (!$json || !empty($json['errcode'])) {
                $this->errCode = $json['errcode'];
     ...
     * @param string $token 手动指定access_token，非必要情况不建议用
     */
    public function checkQiyeAuth($appid = '', $appsecret = '', $token = '')
    {
        if (!$appid || !$appsecret) {
            $appid = $this->appid;
            $appsecret = $this->appsecret;
        ...
        $authname = 'qiye_wechat_access_token' . $appid;
        if ($rs = $this->getCache($authname)) {
            $this->access_token = $rs;
            return $rs;
        }
        $result = $this->http_get(self::API_QIYE_URL_PREFIX . self::AUTH_QIYE_URL . 'corpid=' . $appid . '&corpsecret=' . $appsecret);
        ...
        if ($result) {
            ...
            $expire = $json['expires_in'] ? intval($json['expires_in']) - 100 : 3600;
            $this->user_token = $json['access_token'];
            $this->setCache($authname, $json, $expire);
            //return $this->access_token;
            return $json;
        }
        return false;
     ...
     * @param string $jsapi_ticket 手动指定jsapi_ticket，非必要情况不建议用
     */
    public function getQiyeJsTicket($appid = '', $jsapi_ticket = '')
    {
        if (!$this->access_token && !$this->checkQiyeAuth()) return false;
        if (!$appid) $appid = $this->appid;
        if ($jsapi_ticket) { //手动指定token，优先使用
            $this->jsapi_ticket = $jsapi_ticket;
            return $this->jsapi_ticket;
        }
        $authname = 'wechat_qiye_jsapi_ticket' . $appid;
        if ($rs = $this->getCache($authname)) {
            $this->jsapi_ticket = $rs;
            return $rs;
        }
        $result = $this->http_get(self::API_QIYE_URL_PREFIX . self::GET_QIYE_TICKET_URL . 'access_token=' . $this->access_token);
        if ($result) {
            $json = json_decode($result, true);
            if (!$json || !empty($json['errcode'])) {
                $this->errCode = $json['errcode'];
     ...
     * @return array|bool 返回签名字串
     */
    public function getQiyeJsSign($url, $timestamp = 0, $noncestr = '', $appid = '')
    {
        if (!$this->jsapi_ticket && !$this->getQiyeJsTicket($appid) || !$url) return false;
        if (!$timestamp)
            $timestamp = time();
        if (!$noncestr)
            $noncestr = $this->generateNonceStr();
     ...
     * @return boolean|array
     */
    public function getQiYeDepartmentList()
    {
        if (!$this->access_token && !$this->checkQiyeAuth()) return false;
        $result = $this->http_get(self::API_QIYE_URL_PREFIX . self::QIYE_DEPARTMENT_LIST_URL . 'access_token=' . $this->access_token);
        if ($result) {
            $json = json_decode($result, true);
            if (!$json || !empty($json['errcode'])) {
                $this->errCode = $json['errcode'];
     ...
     *              $data.name 部门名称
     *              $data.parentid 父部门id
     *              $data.order 在父部门中的次序值。order值大的排序靠前。
     *              $data.id 部门id，32位整型
     * @return boolean|array
     */
    public function createQiYeDepartment($data)
    {
        if (!$this->access_token && !$this->checkQiyeAuth()) return false;
        $result = $this->http_post(self::API_QIYE_URL_PREFIX . self::QIYE_DEPARTMENT_CREATE_URL . 'access_token=' . $this->access_token, self::json_encode($data));
        if ($result) {
            $json = json_decode($result, true);
            if (!$json || !empty($json['errcode'])) {
                $this->errCode = $json['errcode'];
     ...
     * @return boolean|array
     */
    public function updateQiYeDepartment($data)
    {
        if (!$this->access_token && !$this->checkQiyeAuth()) return false;
        $result = $this->http_post(self::API_QIYE_URL_PREFIX . self::QIYE_DEPARTMENT_UPDATE_URL . 'access_token=' . $this->access_token, self::json_encode($data));
        if ($result) {
            $json = json_decode($result, true);
            if (!$json || !empty($json['errcode'])) {
                $this->errCode = $json['errcode'];
     ...
     * @return boolean|array
     */
    public function deleteQiYeDepartment($id)
    {
        if (!$this->access_token && !$this->checkQiyeAuth()) return false;
        $result = $this->http_get(self::API_QIYE_URL_PREFIX . self::QIYE_DEPARTMENT_DELETE_URL . 'access_token=' . $this->access_token . '&id=' . $id);
        if ($result) {
            $json = json_decode($result, true);
            if (!$json || !empty($json['errcode'])) {
                $this->errCode = $json['errcode'];
     ...
     * @return boolean|array
     */
    public function sendQiYeMessage($data)
    {
        if (!$this->access_token && !$this->checkQiyeAuth()) return false;
        $data['agentid'] = $this->agentid;
        $result = $this->http_post(self::API_QIYE_URL_PREFIX . self::QIYE_MESSAGE_SEND_URL . 'access_token=' . $this->access_token, self::json_encode($data));
        if ($result) {
            $json = json_decode($result, true);
            if (!$json || !empty($json['errcode'])) {
                $this->errCode = $json['errcode'];
     ...
     * @return bool|mixed
     */
    public function getQiYeAgentInfo()
    {
        if (!$this->access_token && !$this->checkQiyeAuth()) return false;
        $result = $this->http_get(self::API_QIYE_URL_PREFIX . self::QIYE_GET_AGENT_URL . 'access_token=' . $this->access_token . '&agentid=' . $this->agentid);
        if ($result) {
            $json = json_decode($result, true);
            if (!$json || !empty($json['errcode'])) {
                $this->errCode = $json['errcode'];
     ...
     * @return boolean|array
     */
    public function setQiYeAgentInfo($data)
    {
        if (!$this->access_token && !$this->checkQiyeAuth()) return false;
        $data['agentid'] = $this->agentid;
        $result = $this->http_post(self::API_QIYE_URL_PREFIX . self::QIYE_SET_AGENT_URL . 'access_token=' . $this->access_token, self::json_encode($data));
        if ($result) {
            $json = json_decode($result, true);
            if (!$json || !empty($json['errcode'])) {
                $this->errCode = $json['errcode'];
     ...
     * 如果没有UserId和user_ticket则是非企业员工
     */
    public function getQiYeUserInfo()
    {
        $code = isset($_GET['code']) ? $_GET['code'] : '';
        if (!$code) return false;

        if (!$this->access_token && !$this->checkQiyeAuth()) return false;
        $result = $this->http_get(self::API_QIYE_URL_PREFIX . self::QIYE_GET_USERINFO_URL . 'access_token=' . $this->access_token['access_token'] . '&code=' . $code .'&agentid='.$this->agentid);
        if ($result) {
            $json = json_decode($result, true);
            if (!$json || !empty($json['errcode'])) {
                $this->errCode = $json['errcode'];
     ...
     * @return boolean|array
     */
    public function getQiYeUserDetail($user_ticket)
    {
        if (!$this->access_token && !$this->checkQiyeAuth()) return false;

        $result = $this->http_post(self::API_QIYE_URL_PREFIX . self::QIYE_GET_USERDETAIL_URL . 'access_token=' . $this->access_token['access_token'],self::json_encode(['user_ticket' => $user_ticket]));
        if ($result) {
            $json = json_decode($result, true);
            if (!$json || !empty($json['errcode'])) {
                $this->errCode = $json['errcode'];
